...
class DronesControllerTest {
	...
	void loadMedicationNormal() throws Exception {
		...
		String response = mockMvc
				.perform(post(HOST + LOAD_DRONE)
						.contentType(MediaType.APPLICATION_JSON)
						.content(droneMedicationJSON))
				.andExpect(status().isOk())
				.andReturn()
				.getResponse()
				.getContentAsString();
	...
	void getAvailableDrones() throws Exception {
		...
		when(dronesService
				...
				.thenReturn(Arrays.asList(returnedDrones));
	...
	void getLoadedMedicationItems() throws Exception {
		...
		when(dronesService
				.checkMedicationItems(DRONE_NUMBER))
	...
	void getLoadedMedicationsDroneNotFound() throws Exception {
		...
		String response = getRequest(HOST + GET_DRONE_MEDICATIONS + DRONE_NUMBER, 404);
	...
	void getBatteryCapacity() throws Exception {
		int capacity = 100;
		when(dronesService
				.checkBatteryLevel(DRONE_NUMBER))
				.thenReturn(capacity);
	...
	void getLogs() throws Exception {
		LogDto[] returnedLogs = { logDto };
		when(dronesService
				.checkLogs(DRONE_NUMBER))
	...
	void getDronesMedicationsAmounts() throws Exception {
		...
		when(dronesService
				.checkDronesMedicationItemsAmounts())
